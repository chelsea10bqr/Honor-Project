...
NEXT_EVENT
    ...
    BRn SPECIAL           ;check whether time bitvector is negative
    AND R2,R2,#0          ;initialize R2
    BRnzp CHECK_TIMES
SPECIAL                   ;for special case( which the time bitvector is negative)
    AND R2,R2,#0          ;initialize R2 and make it as a counter of time slot value
    ADD R2,R2,#-1
    AND R1,R1,#0
    ADD R1,R1,#1          ;make R1 as a bitmask
BITMASK_CHECK
    ADD R0,R2,#0          ;to store the value of R2 in R0
    AND R3,R4,R1          ;check whether event happens in this time slot
    BRz BITMASK
    ADD R3,R1,R1          ;shift right the bitmask again and store it in R3
    BRz EIGHT             ;if it is zero, which means this is the last time slot
    ADD R1,R3,#0          
    STR R1,R6,#0          ;store the bitmask in the stack
    ADD R2,R2,#1          
    ADD R0,R2,#0 
    ADD R6,R6,#-1         ;push the stack pointer
    BRnzp CHECK_DAYS
EIGHT
    STR R1,R6,#0          ;if it is the last time slot, store the last bitmask in stack
    ADD R2,R2,#1          ; and add 1 to time value counter
    ADD R0,R2,#0
    ADD R6,R6,#-1         ;push the stack
    BRnzp CHECK_DAYS      ;move to check_days
BITMASK                 
    ADD R2,R2,#1          ;add 1 to the counter
    ADD R1,R1,R1          ;shift right the bitmask
    BRnzp BITMASK_CHECK
...
NEXTLOCATION_2            ;if it is not zero, move to next location
    ...
    LDR R4,R5,#2          ;check if the time bitvetor is negative
    BRzp NORMAL           
    LDR R1,R6,#1          ;load the bitmask and check if it is negative 
    BRnz SPECIALF         ;if it is negative check the previous event 
    ADD R2,R0,#0
    BRp BITMASK_CHECK
NORMAL
...
SPECIALF 
    ...
    BRzp CONTINUE_1       ;if it is negative, go to special case
    LDR R1,R6,#1          ;load the bitmask 
    BRn SPECIALF          ;if it is negative, try previous event
    ADD R2,R2,#-2         
    AND R3,R3,#0           
    ADD R4,R1,#0           ;if it is not negative,shift left to determine the time slot value
S    
    ADD R4,R4,#-2
    BRz D
    ADD R3,R3,#1
